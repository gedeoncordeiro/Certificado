...
def create_certificate_overlay(student_name, courses, completion_date, cert_number):
    """
    Cria um PDF temporário com as informações do certificado para sobrepor no PDF principal.
    O conteúdo é centralizado em uma página A4 no modo paisagem e o número do certificado é adicionado.
    ...
    :param cert_number: Número do certificado.
    ...
    # Adiciona o número do certificado no canto superior direito
    c.setFont("Helvetica-Bold", 16)
    c.drawString(page_width - 225, page_height - 545, f": {cert_number}")
...
def main():
    ...
    output_pdf_path = os.path.join(output_dir, "certificado_final2.pdf")

    # Gera o overlay e mescla no PDF principal
    overlay_buffer = create_certificate_overlay(student_name, courses, completion_date, cert_number)
